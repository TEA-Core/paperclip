 ...
 *
 * Lifecycle-bounded: when the tracked run reaches a terminal status the run id
 * is cleared, its accumulated draft is discarded, and the persisted-log poller
 * stops — so a finished run can't linger or pollute a later resubmit. A later
 * run for the same issue is rediscovered (active-run poll or a queued/progress
 * event) and streams from a clean slate; finished run ids are poisoned so the
 * endpoint or a stray event can't re-adopt them.
 *
...
export function useSummaryDraftStream(
  ...
  // has surfaced the run id yet (e.g. a page refresh mid-generation). Discovery
  // polling runs only while a run isn't tracked; it stops once a run is adopted.
  ...
  useEffect(() => {
    if (fallbackRunId && !finishedRunIdsRef.current.has(fallbackRunId)) {
      setRunId((current) => current ?? fallbackRunId);
    }
  }, [fallbackRunId]);
