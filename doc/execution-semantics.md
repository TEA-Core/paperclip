...
This is distinct from productivity review. Productivity review asks whether an assigned source issue has unusual progression patterns, such as no-comment terminal-run streaks, long active duration, or high churn. The no-comment-streak trigger is budget-weighted: each terminal run contributes to the streak in proportion to the run budget it consumed, so cheap cancellations do not over-count against it. Source-resolved watchdog folding asks whether a stale active-run signal outlived a source issue that already reached a valid terminal disposition. One does not substitute for the other.
