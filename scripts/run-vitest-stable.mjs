...
// `shardIndex`/`shardCount` are the fork's CI shard partition (SUP-14712 /
// #499): they are exported to the test process as PAPERCLIP_TEST_SHARD_* so
// port-binding suites can take a per-shard loopback lane. `testShard` is
// upstream #13198's per-test-line split of the chat integration suite. They are
// independent — the chat lane passes both.
function runVitest(args, label, shardIndex = null, shardCount = null, testShard = null) {
  console.log(`\n[test:run] ${label}`);
  invocationIndex += 1;
...
function runGeneralGroup(routeTests, groupName, shardIndex = null, shardCount = null) {
  if (groupName === generalChatGroupName) {
    runVitest(["--project", "@paperclipai/server", ...serializedServerVitestArgs, chatSuite],
      "chat integration test shard", shardIndex, shardCount,
      { index: shardIndex ?? 0, count: shardCount ?? 1 });
    return;
  }
