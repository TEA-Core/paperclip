...
test("install-bearing jobs restore the pr-lockfile artifact only when the policy job regenerated one", () => {
  ...
  // Eight install-bearing jobs (typecheck, server test typecheck, general
  // tests, build, serialized server, canary dry run, e2e shards, worktree uid
  // partition probe) each carry one restore step.
  assert.equal(restores.length, 8, "unexpected number of pr-lockfile restore steps");
