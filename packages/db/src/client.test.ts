...
describeEmbeddedPostgres("applyPendingMigrations", () => {
  it("upgrades renumbered recovery migrations and replays their schema idempotently", async () => {
    ...
    const recoveryFiles = [
      "0265_exotic_dakota_north.sql", "0266_narrow_mastermind.sql",
      "0267_friendly_kate_bishop.sql", "0268_real_firebrand.sql",
      "0269_military_calypso.sql",
    ];
    ...
    try {
      ...
      for (const file of ["0263_small_manta.sql", "0264_fast_silverclaw.sql"]) {
        const hash = await migrationHash(file);
  ...
  it(
    ...
    async () => {
      const connectionString = await createTempDatabase();
      await applyPendingMigrations(connectionString);
      const hash = await migrationHash("0251_remove_cheap_model_profiles.sql");
