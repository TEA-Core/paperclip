...
-- Per constraint, in the same order as 0222's ADD statements:
--   1. VALIDATE the temporary constraint (SHARE UPDATE EXCLUSIVE: the
--      validation scan runs concurrently with writes),
--   2. DROP the old constraint (brief catalog-only lock),
--   3. RENAME the temporary constraint to the canonical name
...
ALTER TABLE "decision_triage_events" RENAME CONSTRAINT "decision_triage_events_agent_api_key_id_agent_api_keys_id_fk_new" TO "decision_triage_events_agent_api_key_id_agent_api_keys_id_fk";--> statement-breakpoint
