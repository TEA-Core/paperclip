...
import { buildGeminiSkillsDir } from "./execute.js";
...
describe.skipIf(process.platform === "win32" || OWN_GID == null)(
  "buildGeminiSkillsDir agent-uid accessibility (SUP-13486)",
  () => {
    let root: string;
    ...
    it("grants the agent child group access to the mkdtemp root and the skills dir", async () => {
      ...
      const skillsDir = await buildGeminiSkillsDir({});
    ...
    it("fails soft when the agents group is absent", async () => {
      ...
      await expect(buildGeminiSkillsDir({})).resolves.toBeTypeOf("string");
    });
  },
);
