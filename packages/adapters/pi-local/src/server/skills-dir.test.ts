...
import { buildPiSkillsDir } from "./execute.js";
...
describe.skipIf(process.platform === "win32" || OWN_GID == null)(
  "buildPiSkillsDir agent-uid accessibility (SUP-13486)",
  () => {
    let root: string;
    ...
    it("grants the agent child group access to the mkdtemp root and the skills dir", async () => {
      ...
      const skillsDir = await buildPiSkillsDir({});
    ...
    it("fails soft when the agents group is absent", async () => {
      ...
      await expect(buildPiSkillsDir({})).resolves.toBeTypeOf("string");
    });
  },
);
