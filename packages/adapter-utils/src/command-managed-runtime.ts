        ...
        for (const [index, mapping] of operation.files.entries()) {
          ...
          try {
            if (mapping.kind === "directory") {
              ...
              // A mode-bearing file is staged, chmod'd, then renamed into place:
              // writing straight to the target would leave it readable at the
              // default mode for the window before the chmod lands.
