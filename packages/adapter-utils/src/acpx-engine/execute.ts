...
/**
 * Fork-only run-timeout diagnostics. When a run times out, the ACP agent child can
 * survive the SIGTERM -- that is exactly the orphan case -- and without this the run
 * reports a bare `acpx_timeout` with no trace of a process still holding the
 * workspace. Upstream has no equivalent, so it is grafted onto upstream's engine
 * here, reading the same process-identity sink upstream already maintains.
 */
...
export interface AcpxEngineExecutorOptions {
   ...
   * the one-shot seed alone cannot hold the group-reachable shape).
   */
...
interface AcpxPreparedRuntime {
  ...
  // Local-lane counterpart; null for remote runs and adapters with no seam.
 ...
 */
// FORK DIVERGENCE — the fork's `sanitizeInheritedPaperclipEnv` denylist is
// deliberately NOT layered on top of this. Upstream's
// `projectAcpxInheritedHostEnvironment` is a closed allowlist, which is strictly
// tighter: a secret the fork's denylist never learned about is dropped here by
// default. `sanitizeInheritedPaperclipEnv` remains the choke point at every
// non-ACPX spawn boundary. See the property assertions in
// `execute.test.ts` ("sanitizeInheritedPaperclipEnv is called at ACPX spawn
// points"), which assert the SECRETS-ABSENT property rather than the call.
...
async function syncBackManagedHome(prepared: AcpxPreparedRuntime): Promise<WorkspaceRestoreOutcome> {
  ...
  const outcome: WorkspaceRestoreOutcome = prepared.remoteManagedHomeTeardown
    ? await prepared
      .remoteManagedHomeTeardown()
      .catch((): WorkspaceRestoreOutcome => ({ ok: false, code: "restore_failed" }))
    : { ok: true };
  // SUP-13503: local-lane counterpart of the remote teardown above —
  // re-normalize the agent-side config home after the CLI's run-end dir
  // creation. It runs on the local lane, where there is no remote teardown, so
  // it sits after the outcome above rather than behind an early return. Same
  // best-effort contract: the seam logs and swallows its own faults, so a
  // re-normalize miss never masks or fails the run result here, and it never
  // changes the restore outcome the caller records.
  ...
  return async function executeAcpxEngine(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    try {
      ...
      const stepTurnFinalize = async (
        ...
        // A timed-out run whose agent child outlived the SIGTERM is an orphan
        // still holding the workspace. Record it as evidence rather than
        // reporting a bare `acpx_timeout`.
        const orphanedProcess = timedOut ? buildOrphanedAgentEvidence(processIdentitySink) : null;
        if (orphanedProcess) {
          await emitAcpxLog(ctx, {
            type: "acpx.error",
            summary: "orphaned_process",
            message:
              `[paperclip] ACP agent process ${orphanedProcess.pid} could not be signaled after the run timed out; `
              + "it is still running unattended.",
          });
        }
        ...
        capturedResult = {
          ...
          timedOut,
          errorMessage,
          // Upstream #13158 promoted the timeout and duplex channel-loss arms ABOVE the
          // failed terminal: once the control channel is lost mid-turn, the provider's own
          // failed terminal is downstream noise and the run must report the loss. SUP-13716
          // only REFINES the failed-terminal arm — `terminalFailureClassification` is
          // non-null exactly when `terminal.status === "failed"`, and classifyError falls
          // back to "acpx_turn_failed", so this arm subsumes upstream's plain failed branch
          // while keeping the Claude OAuth credential escalation.
          errorCode: timedOut
            ? "acpx_timeout"
            : channelLost
