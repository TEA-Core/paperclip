 ...
 */
// `sanitizeInheritedPaperclipEnv` is not optional here. Upstream's version of
// this helper spreads `process.env` raw, which hands the agent child every
// inherited secret the server holds — DATABASE_URL, the signing secrets, the
// secrets master key. The fork strips them at every spawn boundary and has a
// test asserting it; that test is what caught this when upstream's restructured
// bridge path replaced the fork's inline env build.
