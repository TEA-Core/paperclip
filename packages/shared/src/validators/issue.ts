...
export const issueExecutionStagePrincipalSchema =
  issueExecutionStagePrincipalBaseSchema.strict().superRefine((value, ctx) => {
    if (value.type === "agent") {
      if (!value.agentId) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
export const issueExecutionStateSchema = z.object({
  ...
  skippedStageIds: z.array(z.string().guid()).optional().default([]),
    ...
    if (value.outcome === "false_positive" || value.outcome === "cancelled") {
      ...
      if (value.sourceIssueStatus === "blocked") {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            "This recovery outcome requires a non-blocked source issue status",
          path: ["sourceIssueStatus"],
        });
      }
      return;
 ...
 */
export const updateIssueObjectSchema = objectWithoutDefaults(
  ...
  .extend({
    ...
    // `objectWithoutDefaults` rebuilds the shape with a bare `z.object()`, which
    // drops the base schema's `.strict()`. Re-assert it here: the fork keeps the
    // create-only attribution keys IN the shape (they are removed by the
    // `stripCreateOnlyIssueAttribution` transform, so a spoof is a silent strip,
    // not a 400), and strictness is what still makes a typo like `blockedBy` for
    // `blockedByIssueIds` a 400 instead of a silently dropped dependency edge.
  })
  .strict();

export const updateIssueSchema = stripCreateOnlyIssueAttribution(
