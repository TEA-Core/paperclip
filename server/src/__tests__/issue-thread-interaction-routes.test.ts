...
const mockIssueService = vi.hoisted(() => ({
  ...
  addComment: vi.fn(),
  // The terminal-status expiry suites (SUP-10095, SUP-10931) drive the patch and
  // comment routes, which reach these seams on the way to expiring interactions.
  update: vi.fn(),
...
async function loadAppModules() {
  const issues = await import("../routes/issues.js");
  const middleware = await import("../middleware/index.js");
  return [issues, middleware] as const;
}
