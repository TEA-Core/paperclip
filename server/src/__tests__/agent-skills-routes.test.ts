...
describe.sequential("agent skill routes", () => {
  beforeEach(async () => {
    ...
    // concurrently drain vitest's pending-mock queue twice, so this suite's
    // `vi.doMock` replacements can be dropped and a route answers a bare 500
    // (#618; guarded by no-concurrent-module-imports.test.ts).
    ({ agentRoutes } = await vi.importActual<typeof import("../routes/agents.js")>(
      "../routes/agents.js",
    ));
    ({ errorHandler } = await vi.importActual<typeof import("../middleware/index.js")>(
      "../middleware/index.js",
    ));
  });
