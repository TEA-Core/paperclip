...
describeEmbeddedPostgres(
  ...
  () => {
    ...
    it("allows only standard checked-out runs to comment one hop upward", async () => {
      ...
      // FORK BEHAVIOUR (re-asserted at slice 2b, e5d442a8b). The direct-parent
      // report grant is one hop and non-transitive: a grandparent or sibling is
      // not reachable through it, and the fork does not supply upstream's
      // default-open fallback behind it, so these are refused outright rather
      // than admitted and then stopped by the assignee run lock.
