...
// Advance an existing (or new) branch by one commit and push it. The checkout
// keeps whatever the branch already points at — `checkout -B` would reset it to
// HEAD and silently drop the commits these callers are advancing past.
async function advanceRemoteBranch(
  ...
  const branchExists = await runGit(sourceRepo, ["rev-parse", `--verify`, branch])
    .then(() => true)
    .catch(() => false);
  await runGit(sourceRepo, ["checkout", ...(branchExists ? [] : ["-b"]), branch]);
  await fs.writeFile(path.join(sourceRepo, fileName), `${fileName}\n`, "utf8");
  await runGit(sourceRepo, ["add", fileName]);
  await runGit(sourceRepo, ["commit", "-m", `Add ${fileName}`]);
...
describe("waitForRuntimeServiceReadiness", () => {
  ...
  it("retries past a stalled probe instead of spending the whole budget on it", async () => {
    ...
    try {
      ...
      await waitForRuntimeServiceReadiness({
  ...
  it("fast-forwards a clean, behind base repo on the reuse path before provisioning", async () => {
    ...
    const reused = await ensurePersistedExecutionWorkspaceAvailable({
      ...
      workspace: {
        ...
        branchName,
        metadata: { ...RUNTIME_OWNED_GIT_BRANCH_METADATA, baseRefSnapshot: { resolvedSha: initialHead } },
      },
      issue: {
        id: "issue-1",
        identifier: "PAP-460",
