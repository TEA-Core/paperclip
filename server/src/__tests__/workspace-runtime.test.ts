...
import {
  ...
  resolveProvisionChmodOwnershipRepairPath,
  resolveWorkspaceCommandSpawnTarget,
  reownProvisionBuildArtifact,
...
describe("realizeExecutionWorkspace", () => {
  ...
  // SUP-13977: provisioning ran as the server uid (1000) while agent runs write
  // as the agent uid (1001). Both share group `agents`, which shares read and
  // write and cannot share chmod — so pnpm's linkBins aborted the whole install
  // on the first bin the other uid owned, and every dispatch reusing that
  // worktree died as `workspace_validation_failed`.
  //
  // The cross-uid EPERM itself is a kernel property and cannot be reproduced in
  // a single-uid test process; it was verified directly on the deployment. What
  // is tested here is every decision made around it, which is where the bugs
  // would be.
  describe("agent-uid workspace provisioning (SUP-13977)", () => {
    const shell = resolveShell();
    const savedUid = process.env.PAPERCLIP_AGENT_UID;
    const savedShim = process.env.PAPERCLIP_AGENT_SPAWN_SHIM;

    afterEach(() => {
      if (savedUid === undefined) delete process.env.PAPERCLIP_AGENT_UID;
      else process.env.PAPERCLIP_AGENT_UID = savedUid;
      if (savedShim === undefined) delete process.env.PAPERCLIP_AGENT_SPAWN_SHIM;
      else process.env.PAPERCLIP_AGENT_SPAWN_SHIM = savedShim;
    });

    it("leaves commands at the server uid unless the drop is both requested and armed", async () => {
      delete process.env.PAPERCLIP_AGENT_UID;
      const plain = { command: "pnpm install", cwd: "/tmp" };

      // Not requested: teardown and git bookkeeping stay the server's.
      expect(await resolveWorkspaceCommandSpawnTarget(plain)).toEqual({
        command: shell,
        args: ["-c", "pnpm install"],
      });
      // Requested but the deployment has no uid split at all.
      expect(await resolveWorkspaceCommandSpawnTarget({ ...plain, runAsAgentUid: true })).toEqual({
        command: shell,
        args: ["-c", "pnpm install"],
      });
    });

    it("execs through the shim when armed, and refuses to fall back when the shim is missing", async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-shim-"));
      const shimPath = path.join(dir, "paperclip-spawn-agent");
      process.env.PAPERCLIP_AGENT_UID = "1001";
      process.env.PAPERCLIP_AGENT_SPAWN_SHIM = shimPath;

      // A soft fallback here would silently restore the exact mixed-ownership
      // provisioning this change removes, while reporting success.
      await expect(
        resolveWorkspaceCommandSpawnTarget({ command: "pnpm install", cwd: dir, runAsAgentUid: true }),
      ).rejects.toThrow(/setuid spawn shim is not executable/);

      await fs.writeFile(shimPath, "#!/bin/sh\nexec \"$@\"\n", { mode: 0o755 });

      // The shim ends in execvp(argv[1], &argv[1]), so the shell and its -c
      // argument pass through unchanged — no requoting, no extra shell layer.
      expect(
        await resolveWorkspaceCommandSpawnTarget({
          command: "pnpm install",
          resolvedCommand: "bash ./scripts/provision-worktree.sh",
          cwd: dir,
          runAsAgentUid: true,
        }),
      ).toEqual({
        command: shimPath,
        args: [shell, "-c", "bash ./scripts/provision-worktree.sh"],
      });

      await fs.rm(dir, { recursive: true, force: true });
    });

    it("reads the offending path out of pnpm's EPERM, and only inside the worktree", () => {
      const worktree = "/paperclip/instances/default/projects/p/w/paperclip/.paperclip/worktrees/SUP-1";
      const failure = [
        " EPERM  EPERM: operation not permitted, chmod '" + worktree + "/packages/plugins/sdk/dist/dev-cli.js'",
        "    at async chmod (node:internal/fs/promises:1087:10)",
        "    at async linkBin (/paperclip/.cache/node/corepack/v1/pnpm/9.15.4/dist/pnpm.cjs:103822:9)",
      ].join("\n");

      expect(resolveProvisionChmodOwnershipRepairPath(failure, worktree)).toBe(
        worktree + "/packages/plugins/sdk/dist/dev-cli.js",
      );

      // A chmod EPERM outside this worktree is someone else's bug. Repairing a
      // path we were merely told about would be a write primitive driven by
      // subprocess output.
      expect(
        resolveProvisionChmodOwnershipRepairPath(
          "EPERM: operation not permitted, chmod '/etc/paperclip/secrets/master.key'",
          worktree,
        ),
      ).toBeNull();
      expect(
        resolveProvisionChmodOwnershipRepairPath(
          `EPERM: operation not permitted, chmod '${worktree}/../SUP-2/dist/x.js'`,
          worktree,
        ),
      ).toBeNull();
      // Git's bookkeeping is never a build output and never a bin target.
      expect(
        resolveProvisionChmodOwnershipRepairPath(
          `EPERM: operation not permitted, chmod '${worktree}/.git/config'`,
          worktree,
        ),
      ).toBeNull();
      // Not a chmod ownership failure at all.
      expect(resolveProvisionChmodOwnershipRepairPath("ERR_PNPM_OUTDATED_LOCKFILE", worktree)).toBeNull();
      expect(resolveProvisionChmodOwnershipRepairPath("EACCES: permission denied, open '/x'", worktree)).toBeNull();
    });

    it("rewrites an ignored build artifact losslessly and refuses anything git tracks", async () => {
      const repo = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-reown-"));
      await execFileAsync("git", ["init", "-q", repo], { cwd: repo });
      ...
      const artifact = path.join(repo, "dist", "dev-cli.js");
      await fs.writeFile(artifact, "#!/usr/bin/env node\nconsole.log(1)\n", { mode: 0o664 });
      const before = await fs.stat(artifact);

      expect(await reownProvisionBuildArtifact(artifact, repo)).toBe(true);
      // Content and mode survive; only the inode (and so the owner) changes.
      expect(await fs.readFile(artifact, "utf8")).toBe("#!/usr/bin/env node\nconsole.log(1)\n");
      const after = await fs.stat(artifact);
      expect(after.mode & 0o7777).toBe(before.mode & 0o7777);
      expect(after.ino).not.toBe(before.ino);
      // No temp file left behind on the happy path.
      expect((await fs.readdir(path.join(repo, "dist"))).sort()).toEqual(["dev-cli.js"]);

      // A tracked file reaching here is a different bug. Rewriting it under the
      // index would destroy real work to make an install succeed.
      const tracked = path.join(repo, "tracked.js");
      await fs.writeFile(tracked, "source\n");
      await execFileAsync("git", ["add", "tracked.js"], { cwd: repo });
      expect(await reownProvisionBuildArtifact(tracked, repo)).toBe(false);
      expect(await fs.readFile(tracked, "utf8")).toBe("source\n");

      // Symlinks and directories are not the failure being modelled.
      const link = path.join(repo, "dist", "link.js");
      await fs.symlink(artifact, link);
      expect(await reownProvisionBuildArtifact(link, repo)).toBe(false);
      expect(await reownProvisionBuildArtifact(path.join(repo, "dist"), repo)).toBe(false);
