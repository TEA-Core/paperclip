...
async function installActor(
  ...
  // Awaited one at a time, never `Promise.all`: two module graphs resolved
  // concurrently drain vitest's pending-mock queue twice and can drop this
  // suite's mocks, answering a route with a bare 500 (#618; guarded by
  // no-concurrent-module-imports.test.ts).
...
describe.sequential("issue comment reopen routes", () => {
  ...
  it.each([
    ...
    async (kind, sendRequest) => {
      const agentA = "44444444-4444-4444-8444-444444444444";
      // Assigned to the actor: the cross-issue cap is the gate under test, not
      // the assignee-ownership boundary one gate earlier.
      mockIssueService.getById.mockResolvedValue({
        ...makeIssue("todo"),
  ...
  it.each([
    ...
    async (_label, hiddenAt) => {
      const agentA = "44444444-4444-4444-8444-444444444444";
      // Assigned to the actor: the cross-issue counter is the gate under test.
      mockIssueService.getById.mockResolvedValue({
        ...makeIssue("todo"),
        assigneeAgentId: "44444444-4444-4444-8444-444444444444",
        hiddenAt:
  ...
  it("counts bundled cross-issue PATCH updates and comments separately before mutation", async () => {
    const agentA = "44444444-4444-4444-8444-444444444444";
    // Assigned to the actor: the cross-issue counter is the gate under test, not
    // the assignee-ownership boundary one gate earlier.
    mockIssueService.getById.mockResolvedValue({
      ...makeIssue("todo"),
      assigneeAgentId: "44444444-4444-4444-8444-444444444444",
  ...
  describe.each([
    ...
    {
      name: "ticketed approval",
      body:
        "## Review: PAP-580 - APPROVED\n\nClosed at Tier 2 (live): reviewer probe verified the changed surface.",
    },
    {
      name: "lowercase approval",
      body:
