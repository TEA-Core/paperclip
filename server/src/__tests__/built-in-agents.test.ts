...
describeEmbeddedPostgres("built-in agents", () => {
  ...
  it("blocks closing a routine issue while a request_confirmation gate is pending", async () => {
    const companyId = await seedCompany();
  ...
  it("also blocks cancelling a routine issue while a request_confirmation gate is pending", async () => {
    const companyId = await seedCompany();
    const created = await builtInAgentService(db).ensure(companyId, "reflection-coach");
    const coach = created.agent!;
    const routine = await db
    ...
    await expect(issuesSvc.update(routineIssueId, { status: "cancelled" })).rejects.toMatchObject({
      status: 422,
      details: {
        code: "routine_close_pending_confirmation_gates",
        pendingCount: 1,
    ...
    const updated = await issuesSvc.update(routineIssueId, { status: "cancelled" });
