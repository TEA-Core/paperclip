...
describeEmbeddedPostgres("heartbeat issue graph liveness escalation", () => {
  ...
  afterEach(async () => {
    ...
    vi.clearAllMocks();
    // `vi.clearAllMocks()` drops the mocked `loadConfig` return value, and the
    // suite's services read config outside `recoveryServiceWithMocks`; re-arm it
    // so the next test does not start against an undefined config.
    loadConfigMock.mockReturnValue(buildTestConfig());
