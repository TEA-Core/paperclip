...
// Mirrors the route-level schema in routes/issues.ts. `updateIssueSchema` is the
// transformed form (it strips the create-only attribution keys) and so has no
// `.extend`; the route extends the ZodObject and wraps the result the same way.
