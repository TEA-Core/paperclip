...
export type GitCredential = {
  token: string;
  source: "managed_connection" | "app_installation" | "company_secret" | "server_env";
  /** The secret name the token came from; null for a server-environment or managed-connection token. */
 ...
 * resolver — a configured managed identity fails closed instead of falling through to legacy
 * credentials — then the GitHub App installation token scoped to the remote's owner/repo,
 * then company secrets by well-known name, falling through any candidate GitHub rejects
 * (401/403), then the server process env (`GITHUB_TOKEN`/`GH_TOKEN`) for self-hosted
...
export function createGitRemoteAuthProvider(
  ...
  // The managed-identity selection is run-scoped, not repo-scoped: resolve it once per provider
  // so a run performs at most one managed lookup (and at most one OAuth refresh).
  let managedPromise: ReturnType<typeof resolveManagedGitHubCredential> | null = null;
  ...
  const resolveCredential = async (owner?: string, repo?: string): Promise<GitCredential | null> => {
    ...
    // considering the App installation, legacy secrets, or process environment credentials.
    // Only an explicitly configured managed identity (a GitHub tool connection installed for
    // the company or this agent) takes this branch, and it fails closed.
    //
    // `requireEnabledConnection` is the fork's defence in depth (fold decision D1), scoped to
    // THIS git provider and no other consumer: a disabled or non-active GitHub connection must
    // not count as a managed identity here, because `configured: true` with no usable credential
    // throws two lines below and would take every git call in a host-mode run offline on one
    // stale connection row instead of falling through to the App installation token, company
  ...
  return async (remoteUrl: string) => {
    ...
    // SUP-13224: every resolution is scoped to the remote's owner/repo and memoized per
    // owner/repo, so the 401/403 probe fall-through picks a token that actually works for
    // THIS repository instead of reusing the first token the run happened to resolve.
    const ownerRepo = parseOwnerRepo(remoteUrl);
    const key = ownerRepo ? `${ownerRepo.owner}/${ownerRepo.repo}` : "unscoped";
    const resolveScopedCredential = (): Promise<GitCredential | null> => {
      let scoped = credentialPromises.get(key);
      if (!scoped) {
        scoped = resolveCredential(ownerRepo?.owner, ownerRepo?.repo);
        credentialPromises.set(key, scoped);
      }
      return scoped;
    };

    // Upstream #13005 projects the run's managed GitHub identity here and, when that
    // identity is configured but unusable, hands git a deliberately anonymous invocation.
    // The fork runs local and SSH lanes in host mode (fold decision D1) with managed
    ...
    if (!hostGitHub && db && context?.heartbeatRunId && context.agentId) {
      ...
      if (run?.contextId) {
        ...
        if (result.status === "absent") {
          // No managed identity at all: fall back to the fork's owner/repo-scoped chain
          // (App installation -> company secrets -> server env) rather than an unscoped one.
          const credential = await resolveScopedCredential();
...
export async function resolveManagedGitHubIdentitySelection(
  ...
  context: {
    ...
    /**
     * Fork-only, opt-in per caller (fold decision D1): drop disabled/non-active connections from
     * the candidate set so they cannot report `configured: true`. Set ONLY by the git provider in
     * `createGitRemoteAuthProvider`, where a configured-but-unusable managed identity throws and
     * kills the whole git operation. Upstream #13005/#13022 removed this predicate on purpose:
     * without it a revoked or disabled DEDICATED connection stays in the set, keeps
     * `identitySource: "dedicated"` and fails closed instead of silently substituting the
     * responsible person's personal credential. Every other consumer must leave it unset.
     */
    requireEnabledConnection?: boolean;
  },
): Promise<{
  configured: boolean;
  identitySource?: "personal" | "dedicated";
  ...
  const connections = await db.select().from(toolConnections).where(and(
    ...
    ...(context.requireEnabledConnection
...
export async function resolveManagedGitHubCredential(
  ...
  context: {
    ...
    agentId?: string | null;
    allowStandingDelegation?: boolean;
    /** Forwarded verbatim; see `resolveManagedGitHubIdentitySelection`. Only the git provider sets it. */
    requireEnabledConnection?: boolean;
  },
