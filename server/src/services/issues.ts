...
import { pendingReviewParticipantGraceMsFromEnv, pendingReviewRearmWindowMsFromEnv } from "../config.js";
...
export function issueService(db: Db) {
  ...
  const service = {
    ...
    create: async (
      ...
      // When the write runs in a caller-owned transaction (upstream #13038's
      // third parameter) this call cannot see that commit, so the caller hands in
      // its own queue and drains it after committing — the same out-param
      // contract `update` uses.
      const ownedCreateActivityPublications: ActivityPublication[] = [];
      const createActivityPublications =
        postCommitActivityPublications ?? ownedCreateActivityPublications;
    ...
    checkout: async (
      ...
      if (
        issueCompany.status === "done" ||
        issueCompany.status === "cancelled"
      ) {
        throw conflict(
    ...
    addComment: async function addComment(
      ...
      // Fork: redactIssueCommentBody = redactSecretTokens(redactCurrentUserText(...)).
      // Upstream still calls redactCurrentUserText directly; keep the fork's stricter
      // wrapper. Both the insert below and the clientRequestId replay comparison read
      // this same value, so the idempotency check stays consistent.
