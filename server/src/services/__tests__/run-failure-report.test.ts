...
describeEmbeddedPostgres("reportRunFailure", () => {
  ...
  it("removes a database connection string from the error message", async () => {
    ...
    const run = buildRun({
      status: "failed",
      // paperclip:allow-security secret-literal: synthetic fixture, not a credential -- this folded upstream test (#13446) exists to PROVE the redactor strips inline passwords, and the assertion two lines below requires the literal to be absent from the reported message.
