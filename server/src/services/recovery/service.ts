...
export function recoveryService(
  ...
  // The fork's no-output hard-stop (scanTerminableSilentActiveRuns / terminateSilentActiveRun,
  // SUP-9273) still runs here, outside upstream's active-run-watchdog module (#12853). It needs
  // the quiet-until lookup and the process/agent settle helpers the module now keeps private,
  // so the fork's copies stay local to this service.
  ...
  async function ensureSourceScopedStrandedRecoveryAction(input: {
    ...
    const action = await recoveryActionsSvc.upsertSourceScoped({
      ...
      nextAction: recoveryCause === SUCCESSFUL_RUN_MISSING_STATE_REASON
        ...
        : recoveryCause === "configuration_incomplete"
          // Upstream #12957 also reports a not-ready sandbox-provider plugin as
  ...
  async function reconcileStrandedAssignedIssues(opts?: {
    ...
    for (const issue of candidates) {
      ...
      if (armedReviewStageOnNonReviewCard) {
        ...
        // Upstream #13038 keys its chat-control stop arm on the agent being
        // recovered; here that is the review participant, never the assignee.
        if (
          isOperatorCancelledRun(
            await getLatestIssueRun(issue.companyId, issue.id),
            participantOwnerAgentId ?? "",
          )
        ) {
          result.skipped += 1;
          continue;
  ...
  // Upstream #12681 retired the experimental issue-graph liveness auto-recovery (findings ->
  // escalation issues) together with its instance settings and preview/run endpoints. The
  // fork keeps this entry point as the orchestrator of the liveness sweeps that share its
  // timer: the resolved-dependency wake backstop, the dispatch-suppression park (SUP-14881),
  // the deferred-wake replay (SUP-15552), the todo-stranded park (SUP-15553) and the
  // blocked-without-blockers heal. `force`, `lookbackHours` and `reescalationCooldownMs`
  // belonged to the retired escalation pass and are accepted only for caller compatibility;
  // `escalationsCreated` stays in the result, always 0, for the same reason.
