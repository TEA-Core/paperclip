...
export function summarySlotService(db: Db) {
  ...
  function generationIssueDescription(
    ...
    return [
      ...
      `- Write revision: \`PUT ${summarySlotPath}${scopeQuery}\``,
      "",
      "PUT request body — `markdown` is the only required field; unknown fields are rejected:",
      "",
      "- `markdown` (string, required) — the summary text",
      "- `scopeId` (string, optional) — resolved via the query string above",
      "- `title` (string, optional) — revision title",
      "- `changeSummary` (string, optional) — short change note",
      "- `baseRevisionId` (string, optional) — optimistic concurrency guard",
      "- `generationIssueId` (string, optional) — must match the token below",
      "- `model` (string, optional) — model identifier used",
      "",
      "The block below identifies this generation task. The server parses it back out of the issue description and cross-checks it against the slot — it is NOT the request body:",
      "",
      "```json",
      JSON.stringify(
        {
      ...
      "Pass the `generationIssueId` from the token block above, the previous revision id when present, and the model actually used to the summary-slot write API.",
      "",
      scopeSnapshot,
      "",
