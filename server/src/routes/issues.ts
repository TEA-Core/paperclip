...
export function issueRoutes(
  ...
  const enqueueExecutionStageWakeup =
    opts.executionStageWakeupEnqueue ?? heartbeat.wakeup;
  ...
  // SUP-17125: a rejected terminal status write must leave a record the next run on
  // the issue can read, so a close loop ends on the second attempt, not the 27th.
  // The record is a system comment in the issue thread — the surface every run
  // reads — carrying the HTTP status, the guard code, the reason, and the remedy.
  // Deduped per guard code within the last 50 comments so a retry does not stack
  // an identical record.
  const TERMINAL_STATUS_REFUSAL_MARKER = "[Terminal status refused]";

  async function postTerminalStatusRefusalComment(
    issueSvc: ReturnType<typeof issueService>,
    issueId: string,
    input: {
      httpStatus: 409 | 422;
    ...
    const lines = [
      marker,
      "",
      `A terminal status write on this issue (status -> done) was refused with HTTP ${input.httpStatus} (\`${input.code}\`).`,
      `Reason: ${input.error}`,
      `Remedy: ${input.remedy}`,
    ];
  ...
  async function assertCrossIssueInfluenceWithinRunCap(
    ...
    const effectiveRunId =
      req.actor.runId ??
      (await resolveSelfDeclaredRunIdForIssue(
        req.actor.agentId,
        issue.checkoutRunId,
      ));
    if (!req.actor.agentId || !effectiveRunId)
  ...
  async function resolveRunIssueWorkspaceInheritanceSource(
    ...
    const context =
      run.contextSnapshot && typeof run.contextSnapshot === "object"
        ...
        : null;
    const runExecutionWorkspaceId = readNonEmptyString(
    ...
    const workspaceOwner = await db
      ...
      .then((rows) => rows[0] ?? null);
    if (!workspaceOwner || workspaceOwner.sourceIssueId !== runIssueId)
      return null;
    return runIssueId;
  }
  ...
  function isCommentOnlyBoundaryGrant(
  ...
  function commentAuthorizationPathForDecision(
    decision: true | Awaited<ReturnType<typeof decideIssueAccess>>,
  ) {
    if (decision === true) return null;
    if (decision.reason === "allow_manager_chain")
      return "escape_hatch_manager_chain";
  ...
  async function reopenClosedIssueExecutionWorkspaceOrRespond(
    ...
    outcome: "reopened" | "already-open" | "terminal-noop";
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const recoveryActionForHandBack =
      ...
      const handBackAgentId =
        recoveryActionForHandBack?.returnOwnerAgentId ?? null;
      ...
      if (existing.status !== "done" && result.issue.status === "done") {
        try {
          ...
          for (const dependent of dependents) {
            ...
            if (!wakeup) continue;
            const wakeRun = await enqueueRecoveryActionWakeup(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const prDelivery =
        createInput.type === "pull_request"
          ? prDeliverySignature(createInput)
          : null;
      if (prDelivery) {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (
        ...
        return;
      const normalizedAssigneeAgentRef =
      ...
      const actor = getActorInfo(req);
      const requestsWorkspaceInheritanceFromRun =
      ...
      const createBody = {
        ...
        ...(normalizedAssigneeAgentRef.id !== undefined
          ? { assigneeAgentId: normalizedAssigneeAgentRef.id }
          : {}),
        ...(runWorkspaceInheritanceSourceIssueId
          ? {
          ...
          : {}),
        ...(declinedRunWorkspaceInheritance
          ? { executionWorkspacePreference: undefined }
          : {}),
      ...
      const normalizedExecutionPolicy = normalizeIssueExecutionPolicy(
        createBody.executionPolicy,
      );
      const createReturnAssigneeAgentId =
        normalizedExecutionPolicy?.returnAssigneeAgentId ?? null;
      const hasAgentAssignee =
        Boolean(rawCreateBody.assigneeAgentId) ||
        Boolean(rawCreateBody.assigneeUserId) ||
        Boolean(createReturnAssigneeAgentId);
      const assigneeAgentName =
        normalizedAssigneeAgentRef.id != null
          ? normalizedAssigneeAgentRef.name
          : null;
      const isCoderAgent =
        assigneeAgentName != null &&
        assigneeAgentName.toLowerCase().startsWith("coder-");
      if (
        isCoderAgent &&
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const childReturnAssigneeAgentId =
        normalizedExecutionPolicy?.returnAssigneeAgentId ?? null;
      const hasAgentAssignee =
        Boolean(sanitizedBody.assigneeAgentId) ||
        Boolean(sanitizedBody.assigneeUserId) ||
        Boolean(childReturnAssigneeAgentId);
      const childAssigneeAgentName =
        normalizedAssigneeAgentRef.id != null
          ? normalizedAssigneeAgentRef.name
          : null;
      const isChildCoderAgent =
        childAssigneeAgentName != null &&
        childAssigneeAgentName.toLowerCase().startsWith("coder-");
      if (
        isChildCoderAgent &&
        (!normalizedExecutionPolicy ||
          normalizedExecutionPolicy.stages.length === 0)
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (issue.status === "done" || issue.status === "cancelled") {
        throw conflict(
          "Issue cannot be checked out because it is already closed",
          {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const agentSourceRunId =
        req.actor.type === "agent"
          ? await requireAgentRunId(req, res, {
              checkoutRunId: issue.checkoutRunId,
            })
          : null;
      ...
      if (resolvedContinuationIssue) {
        await logActivity(db, {
          ...
          details: {
            ...
            ...(resolvedContinuationIssue.workMode
              ? { workMode: resolvedContinuationIssue.workMode }
              : {}),
    ...
    async (req, res) => {
      ...
      if (
        ...
        const reason =
          ...
          interaction.result.reason.trim().length > 0
