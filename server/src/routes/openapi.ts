...
const BOARD_ONLY_OPERATIONS = new Set([
  // Added upstream in the 2026-09-19 slice-2d fold range (#13284, Agent Chat).
  // Every ai-connections handler calls assertBoard() unconditionally, so the
  // fork's SUP-16341 regression guard requires them listed here.
  ...
  // Added upstream in the 2026-09-15 slice-2c range (#13275). The handler calls
  // assertBoard() unconditionally and also requires a board user context.
  ...
  // Added upstream in the 2026-09-12 slice-2c fold range. Both handlers assert a
  // board actor unconditionally.
  "GET /api/companies/{companyId}/project-repositories",
  "PUT /api/projects/{id}/repositories",
  // Added upstream in the 2026-09-11 slice-2b fold range (#13003). The handler
  // rejects every non-board actor unconditionally, so it is board-only.
...
// ─── Task drain (upstream run-admission hold) ────────────────────────────────
