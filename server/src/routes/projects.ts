...
export function projectRoutes(db: Db) {
  ...
  /**
   * SUP-15614 / SUP-15722: the sanctioned, board-only operator path for a base-repo
   * rescue reset. Delegates the destructive sequence to the lease-backed
   * `resetProjectBaseRepoWithRescue` (keyed by `baseRef`) and maps its
   * `BaseRepoRescueResetResult` — a refusal → 409, a success → the reset fields.
   * Every attempt is audit-logged before and after the destructive move.
   */
  router.post("/projects/:id/workspaces/:workspaceId/base-repo/rescue-reset", validate(baseRepoRescueResetSchema), async (req, res) => {
    ...
    const result = await resetProjectBaseRepoWithRescue({ repoRoot, baseRef: targetRef });
    ...
    await logActivity(db, {
      ...
      details: {
        ...
        targetRef,
        ok: result.ok,
        alreadyAtTarget: result.ok ? result.alreadyAtTarget : null,
        resetToSha: result.ok ? result.resetToSha : null,
        previousTip: result.ok ? result.previousTip : null,
        rescueRef: result.ok ? result.rescueRef : null,
        reason: result.ok ? reason || null : result.reason,
    ...
    if (!result.ok) {
      res.status(409).json({ error: result.detail, ...result });
      return;
    }
    res.json(result);
  });
