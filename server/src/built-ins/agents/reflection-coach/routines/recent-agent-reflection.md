...
> **CRITICAL — pending confirmation gate rule**: The server enforces this. If this routine issue has any `request_confirmation`, `request_checkbox_confirmation`, `request_item_verdicts`, or `ask_user_questions` interaction in `pending` status, the issue **cannot** be marked `done` or `cancelled` by you. The server rejects the status transition with code `routine_close_pending_confirmation_gates`. Resolve every pending gate (accept or reject), or leave the issue `in_progress` / `blocked` with the pending interaction named as the unblocker. Closing the issue while any of those gates is pending expires it.
